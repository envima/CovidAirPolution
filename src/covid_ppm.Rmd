...
flist=list.files(file.path(envrmt$`path_report-data-platform-16229-259611-lombardy`), pattern = "^.*\\.csv$", full.names = TRUE)
  ...
  #act$lon=as.numeric(latlon[2])
  #act$lat=as.numeric(latlon[1])
  act$statname<-substr(basename(f), regexec("\\.", basename(f))[[1]][1]+1, regexec("--", basename(f))[[1]][1]-1)
  act = act[act$date >= as.Date("2020-02-01"), ]
  p = sf::st_sfc(sf::st_point(c(as.numeric(latlon[2]),as.numeric(latlon[1]))))
  act=(sf::st_sf(act,p,crs = 4326))
  #act = list(act)
...
pm_stations=sf::st_as_sf(data.table::rbindlist(aq,fill = TRUE)) 
mapview::mapview(pm_stations,zcol="pm25")     
