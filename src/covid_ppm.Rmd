...
de_nuts3_mean = germany$de_nuts3_mean
```
...
for(l in seq(length(it_nuts3_mean))){
  ...
  g = ggplot() +
    geom_line(data = it_nuts3_mean[[l]], aes(x = date, y = pm25_mean, colour  = "PM 2.5 mean")) +
    geom_line(data = it_nuts3_mean[[l]], aes(x = date, y = new_cases/10, colour  = "New cases")) +
    scale_y_continuous(sec.axis = sec_axis(~.*10, name = "New cases")) +
    labs(title = names(it_nuts3_mean[l]), x = "Date", y = "PM 25 mean") +
