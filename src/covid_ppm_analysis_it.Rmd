...
# })
# 
pm25_mean = lapply(nuts3_mean, function(n){
...
# 
# 
# de_clstr_pm25_map = lapply(seq(length(nuts3_mean)), function(c){
#   tmp = nuts3_mean[[c]][1, ]
#   tmp$cluster = as.factor(paste0("c ", pm25_dtw_cluster@cluster[c], " n=(", 
#                                  pm25_dtw_cluster@clusinfo[pm25_dtw_cluster@cluster[c], 1],
#                                  ")"))
#   return(tmp)
# })
# de_clstr_pm25_map = do.call("rbind", de_clstr_pm25_map)
# 
# mapview(de_clstr_pm25_map, zcol = "cluster")
...
de_clstr = lapply(seq(length(nuts3_mean)), function(c){
  tmp = nuts3_mean[[c]][, c("date", "pm25_mean", "nuts3Code", "new_cases", 
                            "cases")]
  tmp$pm25_cluster = as.factor(paste0("c ", pm25_dtw_cluster@cluster[c], " n=(", 
                                      pm25_dtw_cluster@clusinfo[pm25_dtw_cluster@cluster[c], 1],
                                      ")"))
  return(tmp)
})
...
de_clstr$pm25_cluster = factor(de_clstr$pm25_cluster, levels = sort(unique(as.character(de_clstr$pm25_cluster))))

de_clstr_avg = de_clstr[, -which(names(de_clstr) == "nuts3Code")]
de_clstr_avg = aggregate(. ~ date + pm25_cluster, data = de_clstr_avg, FUN = mean)
...
#   geom_line(data = de_clstr_avg, aes(x = date, y = new_cases, color = "New cases")) +
...
#   geom_vline(xintercept = as.POSIXct("2020-03-14"), linetype="dotted", color = "black") +
#   geom_vline(xintercept = as.POSIXct("2020-03-17"), linetype="dotted", color = "black") +
#   facet_wrap(~pm25_cluster, ncol =2, nrow = 2, scales = "free")


de_clstr_avg_detr = lapply(unique(de_clstr_avg$pm25_cluster), function(c){
  print(c)
  tmp = de_clstr_avg[de_clstr_avg$pm25_cluster == c,]
  tmp$new_cases[tmp$new_cases < 0] = 0
  detr = glm(new_cases ~ date,  family = poisson, data =tmp)
  de_clstr_avg[de_clstr_avg$pm25_cluster == c, "new_cases_detr"] = residuals(detr)
  return(de_clstr_avg[de_clstr_avg$pm25_cluster == c,])
})
...
ggplot() + 
  ...
  geom_line(data = de_clstr_avg_detr, aes(x = date, y = new_cases, color = "New cases (smoothed)")) + 
  geom_line(data = de_clstr_avg_detr, aes(x = date, y = new_cases_detr*5, color = "New cases (smoothed, detrended)")) +
  labs(title = "PM2.5 x daily new Covid-19 cases (smoothed, detrended)", x = "Date", y = "Value") +
  scale_x_datetime(date_labels = "%d.%m - %a", date_breaks = "1 day") + 
  scale_color_manual(values=c("red", "darkgreen", "blue")) +
  theme(axis.text.x = element_text(angle=90)) + 
  ...
  geom_vline(xintercept = as.POSIXct("2020-03-17"), linetype="dotted", color = "black") +
  theme(legend.title = element_blank()) +
  facet_wrap(~pm25_cluster, ncol =2, nrow = 2, scales = "fix")
```
...
de_clstr_indv_lag = lapply(unique(de_clstr_indv$nuts3Code), function(n){
  ...
  de_clstr_indv_lag = lapply(seq(0, 14), function(l){
    ...
    data.frame(nuts3_code = n,
               cluster = NA,
               pm25_cluster = tmp$pm25_cluster[1],
...
```{r, eval = TRUE, echo=FALSE, message=FALSE, warning=FALSE, results = FALSE}
ggplot(data = de_clstr_indv_lag, aes(x = as.factor(lag), y = t, fill = pm25_cluster)) + 
  geom_boxplot() + 
  geom_hline(yintercept = c(-2, 2), linetype="dashed", color = "red") + 
  labs(title = "Dependence of individual COVID-19 time series and lagged PM2.5 concentrations by clusters",x = "Time lag PM2.5 [days]", y = "t value of the partial coefficient")
```
...
max_pm25 = aggregate(pm25_mean ~ nuts3Code, data = nr, FUN = mean)
test_area_max_pm25 = merge(test_area[, -which(colnames(test_area) %in% c("date", "pm25_mean"))]
, max_pm25)
t.test(pm25_mean ~ small_p, data = test_area_max_pm25)




test_geo = merge(de_clstr_indv_lag, nuts3_map[, c("nuts3Code", "cases", "nuts3Name", "stato", "lat", "lon", "nuts3Area")], by.x = "nuts3_code", by.y = "nuts3Code")
test_geo = test_geo[test_geo$lag == -6,]
test_geo_lm = lm(test_geo$t ~ test_geo$lat + test_geo$lon)
summary(test_geo_lm)
...
```{r, eval = FALSE, echo=FALSE, message=FALSE, warning=FALSE, results = FALSE}
ggplot(data = de_clstr_indv_lag[de_clstr_indv_lag$pm25_mean_mean > 9, ], aes(x = as.factor(lag), y = t)) + 
  geom_boxplot() + 
  geom_hline(yintercept = c(-2, 2), linetype="dashed", color = "red") + 
  labs(title = "Dependence of individual COVID-19 time series and lagged PM2.5 concentrations by clusters",x = "Time lag PM2.5 [days]", y = "t value of the partial coefficient")
```
