...
pdf("nuts_individual dynamics.glm.pdf")
...
for (i in (1:length(n.areas))) {
  ...
  if (dim(data[[i]])[1] > covid.end) {
    for (k in (0:time.lag)) {
      result.lag <- glm(new_cases[covid.start:covid.end] ~ 
                        c(covid.start:covid.end) +
                        weekday[covid.start:covid.end] +  
 #                      weekday_c[covid.start:covid.end] +   
                        pm_mean[(covid.start-k):(covid.end-k)], 
                        quasipoisson, data = data[[i]])
      result.areas[i,k+1] <- summary(result.lag)$coefficients[5,3]
      result.areas.R2[i,k+1] <- result.lag$deviance/
                                result.lag$null.deviance
      if (k==0)
