---
title: "PM concentrations and SARS-CoV-2 infections over cmpldata"
...
# Compile comprehensive analysis data or load preprocessed dataset.
if (pm == "PM2.5") {
  cmpldata_file <- "germany_extended_025.RDS"
} else {
  cmpldata_file <- "germany_extended_100.RDS"
}

if (compile_data) {
  cmpldata <- compileDataDE(start_date = start_date, end_date = end_date, pm = pm)
  saveRDS(cmpldata, file.path(envrmt$path_tmp, cmpldata_file))
} else {
  cmpldata <- readRDS(file.path(envrmt$path_tmp, cmpldata_file))
}
...
This analysis part is based on countrywide averaged data. A GAM model is computed to predict the daily new cases explained by time. The two figures show the mean daily PM and covid cases dynamics as well as the daily new cases and residuals if time is used as explanatory variable for SARS-CoV2 infections.

```{r, eval = TRUE, echo=FALSE, message=FALSE, warning=FALSE, results = FALSE}

cntry_avg <- cmpldata$de_avg
...
ggplot(cntry_indv[cntry_indv$nuts3Code != 5162,], aes(x = date, y = pm_median, group = nuts3Code)) + geom_line()

figure_cntry_avg <- ggplot() +
  geom_line(data = cntry_avg_gam, aes(x = date, y = pm_median, color = "Daily median PM10")) +
  geom_point(data = cntry_avg_gam, aes(x = date, y = pm_median, color = "Daily median PM10")) +
...
This analysis part is based on individual rural district data. A GAM model is computed to evaluate the influence of lagged PM concentrations on daily new SARS-CoV2 infections. The figure shows the effect size of the model for the individual time lags.
...
if (compile_models) {
  gam_lag <- compileLaggedGAM(
    data = cntry_indv, lag_var = "pm_median",
    frml = "new_cases ~ s(seq(length(date))) + weekday + pm_median_lag",
    nlags = 15,
    subset_var = "new_cases", subset_thv = 10, individual = "start", ndays = c(0, 30)
  )
  saveRDS(gam_lag, file.path(envrmt$path_tmp, "gam_lag.rds"))
} else {
  gam_lag <- readRDS(file.path(envrmt$path_tmp, "gam_lag.rds"))
}
...
figure_effectsize <- ggplot(data = gam_lag, aes(x = as.factor(lag), y = t, fill = fillcol, alpha = fillcol)) +
  geom_boxplot() +
  theme_bw() +
...
# gamm: frml = "new_cases ~ s(date_seq) + weekday + pm_median_lag + offset(pop_total_log10)"
# gam: frml = 'new_cases ~ s(date_seq) + weekday + pm_median_lag + s(nuts3CodeFactor, bs="re")'
mlm <- "gamm"
if (mlm == "gamm") {
...
if (compile_models) {
  gam_lag_mixed <- compileLaggedMixedModel(
    data = cntry_indv, pm = "org",
    frml = 'new_cases ~ s(date_seq) + weekday + pm_median_lag + s(nuts3CodeFactor, bs="re")',
    nlags = 15,
    subset_var = "new_cases", subset_thv = 10, individual = "start", ndays = c(0, 30), model = mlm
  )
  saveRDS(gam_lag_mixed, file.path(envrmt$path_tmp, paste0("gam_lag_mixed_", mlm, ".rds")))
} else {
  gam_lag_mixed <- readRDS(file.path(envrmt$path_tmp, paste0("gam_lag_mixed_", mlm, ".rds")))
}
...
figure_estimate_mixed <- ggplot(
  ...
  theme_bw() +
  geom_hline(yintercept = c(0), linetype = "dashed", color = "black") +
  labs(x = "Time lag [days]", y = "Estimate") +
  theme(
    text = element_text(size = 10), axis.title = element_text(size = 10),
    panel.background = element_blank(), panel.grid.major = element_blank(),
    panel.grid.minor = element_blank()
...
tmp <- cmpldata$de_clstr$clstr[cmpldata$de_clstr$clstr$date <= as.POSIXct("2020-04-01"), ]

cntry_agg <- aggregate(tmp$pm_median, by = list(tmp$nuts3Code), FUN = mean)
...
map_pm_median <- ggplot(data = background_map) +
  ...
  labs(fill = "Median PM 15.02.2020 to 01.04.2020") +
  coord_sf(
    xlim = c(
      (st_bbox(layer)["xmin"] - 10000),
      (st_bbox(layer)["xmax"] + 10000)
