...
de_avg_glm_time_03 = de_avg[de_avg$date >= as.POSIXct("2020-03-01") & de_avg$date <= as.POSIXct("2020-04-01"),]
set.seed(01042020)
de_avg_glm_time = glm(new_cases ~ date + weekday , family = quasipoisson, data = de_avg_glm_time_03)
...
figure_02 = ggplot() + 
  geom_line(data = de_avg_glm_time_03, aes(x = date, y = residuals, color = "Daily new cases, residuals")) +
  geom_point(data = de_avg_glm_time_03, aes(x = date, y = residuals, color = "Daily new cases, residuals")) +
  geom_vline(xintercept = as.POSIXct("2020-03-14"), linetype="dotted", color = "black") +
  geom_vline(xintercept = as.POSIXct("2020-03-17"), linetype="dotted", color = "black") +
...
ggplot() + 
  geom_line(data = de_clstr$clstr_avg_pm_glm_time, aes(x = date, y = pm_mean, color = "Daily mean PM10")) + 
  geom_line(data = de_clstr$clstr_avg_pm_glm_time, aes(x = date, y = new_cases, color = "Daily new cases")) + 
  geom_line(data = de_clstr$clstr_avg_pm_glm_time, aes(x = date, y = new_cases_glm_time, color = "Daily new cases, explained by time")) +
  geom_vline(xintercept = as.POSIXct("2020-03-14"), linetype="dotted", color = "black") +
  geom_vline(xintercept = as.POSIXct("2020-03-17"), linetype="dotted", color = "black") +
  labs(x = "Date and day of week", y = expression("Infections, PM10 ~µm/m^{3}")) +
  theme_bw() + 
...
ggplot() + 
  geom_line(data = de_clstr$clstr_avg_covid_glm_time, aes(x = date, y = pm_mean, color = "Daily mean PM10")) + 
  geom_line(data = de_clstr$clstr_avg_covid_glm_time, aes(x = date, y = new_cases, color = "Daily new cases")) + 
  geom_line(data = de_clstr$clstr_avg_covid_glm_time, aes(x = date, y = new_cases_glm_time, color = "Daily new cases, explained by time")) +
  geom_vline(xintercept = as.POSIXct("2020-03-14"), linetype="dotted", color = "black") +
  geom_vline(xintercept = as.POSIXct("2020-03-17"), linetype="dotted", color = "black") +
  labs(x = "Date and day of week", y = expression("Infections, PM10 ~µm/m^{3}")) +
  theme_bw() + 
...
tstat = lapply(seq(0, -20), function(i){
...
figure_03 =  ggplot(data = de_model_lag, aes(x = as.factor(lag), y = t)) + 
  geom_boxplot() + 
  theme_bw() +
...
set.seed(01042020)
rd = rda(t_matrix[, 2:22], t_matrix[, c("pm_mean", "pop_dens", "cases_at_max", "centroid_lat")])
...
core_geo = cbind(t_matrix$nuts3Code, scores(rd)$sites)
...
# SARS-CoV2 infections on 01.04.2020
background_map = st_transform(ne_countries(scale = "large", continent = "europe", returnclass = "sf"),
                             crs = "+init=epsg:25832")
layer = st_transform(germany$de_nuts3_map, crs = "+init=epsg:25832")
figure_05 = ggplot(data = background_map) +
  geom_sf() + 
...
figure_06 = ggplot(data = background_map) +
  geom_sf() + 
  geom_sf(data = layer) +
  geom_sf(data = layer1, aes(fill = lag)) +
  scale_fill_viridis_c() +
  theme(legend.position="bottom") +
  labs(fill = "Max. lag of positive effect size") +
  coord_sf(xlim = c((st_bbox(layer)["xmin"] - 10000), 
                    (st_bbox(layer)["xmax"] + 10000)), 
           ylim = c((st_bbox(layer)["ymin"] - 25000),
                    (st_bbox(layer)["ymax"] + 10000)), expand = FALSE) + 
...
figure_2_out = plot_grid(figure_03, figure_04, figure_05, figure_06, 
                         labels = "auto", label_size = 10, ncol = 2)
ggsave(file.path(envrmt$path_figures, "figure_2_new.png"), plot = figure_2_out, 
