...
This analysis part is based on countrywide averaged data. A GAM model is computed to predict the daily new cases explained by time. The figure shows the mean daily PM and covid cases dynamics as well as the daily new cases if time is used as explanatory variable for SARS-CoV2 infections.
...
figure_cntry_avg <- ggplot() +
  ...
  labs(x = "Date and day of week", y = bquote("Infections, " ~ .(pm) ~ " [" ~ µm / m^3 ~ "]")) +
  theme_bw() +
...
if (compile_models) {
  gam_lag <- lapply(lag_vars_set, function(lag_var) {
    ...
    saveRDS(gam_lag, file.path(envrmt$path_analysis, paste0(pm, "_gam_lag_", lag_var, "_.rds")))
  })
  names(gam_lag) <- lag_vars_set
  saveRDS(gam_lag, file.path(envrmt$path_analysis, paste0(pm, "_gam_lag_lag_vars_set.rds")))
  return(gam_lag)
} else {
  gam_lag_set <- readRDS(file.path(envrmt$path_analysis, paste0(pm, "_gam_lag_lag_vars_set.rds")))
}
...
figure_gam_lag_set <- lapply(names(gam_lag_set), function(n) {
  ...
  figure_gam_effectsize <- ggplot(data = gam_lag, aes(x = as.factor(lag), y = t, fill = fillcol, alpha = fillcol)) +
    ...
    labs(x = "Time lag [days]", y = paste0("Effect size, ", pm, " ", lag_vars_set_lut[n])) +
    theme(
      text = element_text(size = 10), axis.title = element_text(size = 10),
...
if (compile_models) {
  gam_lag_mixed <- lapply(lag_vars_set, function(lag_var) {
    ...
    saveRDS(gam_lag_mixed, file.path(envrmt$path_analysis, paste0(pm, "_gam_lag_mixed_", mlm, "_", lag_var, ".rds")))
  })
  names(gam_lag_mixed) <- lag_vars_set
  saveRDS(gam_lag_mixed, file.path(envrmt$path_analysis, paste0(pm, "_gam_lag_mixed_lag_vars_set.rds")))
  return(gam_lag_mixed)
} else {
  gam_lag_mixed_set <- readRDS(file.path(envrmt$path_analysis, paste0(pm, "_gam_lag_mixed_lag_vars_set.rds")))
}
...
figure_gam_lag_mixed_set <- lapply(names(gam_lag_mixed_set), function(n) {
  ...
  figure_estimate_mixed <- ggplot(
    ...
    theme_bw() +
    geom_hline(yintercept = c(0), linetype = "dashed", color = "black") +
    labs(x = "Time lag [days]", y = paste0("Estimate, ", pm, " ", lag_vars_set_lut[n])) +
    theme(
      text = element_text(size = 10), axis.title = element_text(size = 10),
      panel.background = element_blank(), panel.grid.major = element_blank(),
      panel.grid.minor = element_blank()
