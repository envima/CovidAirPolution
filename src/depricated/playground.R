...
# create mapview data
pm<-lapply(aq, function(x) x[,1:4])
for (i in 1:length(pm))names(pm[[i]])<-c("data","pm","pmmin","pmmax")
pop = lapply(pm, htmlTable)
mapview(pts, popup = pop,legend = FALSE, ncol="pmmax")
...
# ggplot() + 
#   geom_line(data = cmb, aes(x = date, y = pm, colour = stat)) + 
...
fig = plot_ly() 
for(l in seq_along(pm)){
  
  fig = fig %>% add_trace(data = pm[[l]], x = ~date, y = ~pm, type = 'scatter', name = paste("pm 2.5",waqnames[l]), mode = 'lines+markers')
...
# merge data colums
bergamo<-merge(x = cov_admin3[cov_admin3$denominazione_provincia == "Bergamo",], y = pm[[8]], by = "data",sort = T)
head(bergamo)
plot(bergamo$new_cases,type = "l")
plot(bergamo$pm,type = "l")
...
bergamo.wc <- analyze.coherency(bergamo,my.pair = c("new_cases", "pm"),
