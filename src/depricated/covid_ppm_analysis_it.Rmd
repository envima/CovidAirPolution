...
# 
# 
# de_clstr_pm_map = lapply(seq(length(nuts3_mean)), function(c){
#   tmp = nuts3_mean[[c]][1, ]
#   tmp$cluster = as.factor(paste0("c ", pm_dtw_cluster@cluster[c], " n=(", 
#                                  pm_dtw_cluster@clusinfo[pm_dtw_cluster@cluster[c], 1],
#                                  ")"))
#   return(tmp)
# })
# de_clstr_pm_map = do.call("rbind", de_clstr_pm_map)
# 
# mapview(de_clstr_pm_map, zcol = "cluster")
...
de_clstr = lapply(seq(length(nuts3_mean)), function(c){
  tmp = nuts3_mean[[c]][, c("date", "pm_mean", "nuts3Code", "new_cases", 
                            "cases")]
  tmp$pm_cluster = as.factor(paste0("c ", pm_dtw_cluster@cluster[c], " n=(", 
                                      pm_dtw_cluster@clusinfo[pm_dtw_cluster@cluster[c], 1],
                                      ")"))
  return(tmp)
})
...
de_clstr$pm_cluster = factor(de_clstr$pm_cluster, levels = sort(unique(as.character(de_clstr$pm_cluster))))

de_clstr_avg = de_clstr[, -which(names(de_clstr) == "nuts3Code")]
de_clstr_avg = aggregate(. ~ date + pm_cluster, data = de_clstr_avg, FUN = mean)
...
#   geom_vline(xintercept = as.POSIXct("2020-03-14"), linetype="dotted", color = "black") +
#   geom_vline(xintercept = as.POSIXct("2020-03-17"), linetype="dotted", color = "black") +
#   facet_wrap(~pm_cluster, ncol =2, nrow = 2, scales = "free")


de_clstr_avg_detr = lapply(unique(de_clstr_avg$pm_cluster), function(c){
  print(c)
  tmp = de_clstr_avg[de_clstr_avg$pm_cluster == c,]
  tmp$new_cases[tmp$new_cases < 0] = 0
  detr = glm(new_cases ~ date,  family = poisson, data =tmp)
  de_clstr_avg[de_clstr_avg$pm_cluster == c, "new_cases_detr"] = residuals(detr)
  return(de_clstr_avg[de_clstr_avg$pm_cluster == c,])
})
...
ggplot() + 
  ...
  geom_vline(xintercept = as.POSIXct("2020-03-17"), linetype="dotted", color = "black") +
  theme(legend.title = element_blank()) +
  facet_wrap(~pm_cluster, ncol =2, nrow = 2, scales = "fix")
```
