...
mapview(de_clstr_map, zcol = "cluster_pm25")
...
de_clstr_avg = aggregate(. ~ date + cluster_pm25, data = de_clstr_avg, FUN = mean)
...
de_clstr_avg_detr = lapply(unique(de_clstr_avg$cluster_pm25), function(c){
  tmp = de_clstr_avg[de_clstr_avg$cluster_pm25 == c,]
  detr = glm(new_cases_smooth ~ date,  family = poisson, data =tmp)
  de_clstr_avg[de_clstr_avg$cluster_pm25 == c, "new_cases_smooth_detr"] = residuals(detr)
  return(de_clstr_avg[de_clstr_avg$cluster_pm25 == c,])
})
...
```{r, eval = TRUE, echo=FALSE, message=FALSE, warning=FALSE, results = FALSE}
for(c in unique(de_clstr_avg$cluster_pm25)){
  tmp = de_clstr_avg_detr[de_clstr_avg_detr$cluster_pm25 == c,]
  set.seed(01042020)
...
```{r, eval = TRUE, echo=FALSE, message=FALSE, warning=FALSE, results = FALSE}
ggplot(data = de_clstr_indv_lag, aes(x = as.factor(lag), y = t, fill = cluster_pm25)) + 
  geom_boxplot() + 
  theme_bw() + 
  geom_hline(yintercept = c(-2, 2), linetype="dashed", color = "red") + 
