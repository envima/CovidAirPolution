...
# Load Italy -------------------------------------------------------------------
if(compile_data){
  italy = compileDataIT(start_date = start_date, end_date = end_date)
...
it_avg = italy$it_avg
ggplot() + 
  geom_line(data = it_avg, aes(x = date, y = pm25_mean, color = "PM2.5")) + 
...
mapview(italy$it_clstr$clstr_map, zcol = "cluster_pm25")
```
...
ggplot() + 
  geom_line(data = it_clstr$clstr_avg_detr, aes(x = date, y = pm25_mean, color = "PM2.5")) + 
...
```{r, eval = TRUE, echo=FALSE, message=FALSE, warning=FALSE, results = FALSE}
it_clstr = italy$it_clstr
for(c in unique(it_clstr$clstr_avg_detr$cluster_pm25)){
  tmp = it_clstr$clstr_avg_detr[it_clstr$clstr_avg_detr$cluster_pm25 == c,]
  wc = compileWCIT(tmp)
...
```{r, eval = TRUE, echo=FALSE, message=FALSE, warning=FALSE, results = FALSE}
ggplot(data = it_model_lag, aes(x = as.factor(lag), y = t, fill = cluster_pm25)) + 
  geom_boxplot() + 
  theme_bw() + 
  geom_hline(yintercept = c(-2, 2), linetype="dashed", color = "red") + 
