...
for(pm in pm_vars){
  ...
  gam_lag <- lapply(lag_vars_set, function(lag_var){
    ...
    saveRDS(gam_lag, file.path(envrmt$path_analysis, paste0("italy_", pm, "_gam_lag_", lag_var, ".rds")))
    return(gam_lag)
  })
  names(gam_lag) <- lag_vars_set
  saveRDS(gam_lag, file.path(envrmt$path_analysis, paste0("italy_", pm, "_gam_lag_vars_set.rds")))
  ...
  gamm_lag_mixed <- lapply(lag_vars_set, function(lag_var) {
    ...
    saveRDS(gamm_lag_mixed, file.path(envrmt$path_analysis, paste0("italy_", pm, "_gamm_lag_mixed_", lag_var, ".rds")))
    return(gamm_lag_mixed)
  })
  names(gamm_lag_mixed) <- lag_vars_set
  saveRDS(gamm_lag_mixed, file.path(envrmt$path_analysis, paste0("italy_", pm, "_gamm_lag_mixed_vars_set.rds")))
}
