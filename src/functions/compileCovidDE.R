#' Compile SARS-CoV-2 data for Germany.
#'
#' COVID-19 data is from the GitHub project jgehrcke/covid-19-germany-gae.
#' Daily new infection cases are computed from the daily total sum.
#' Geographic information is included based on NUTS-3 names and their
#' geographical extent.

compileCovidDE <- function() {
  ...
  download.file(
    url = "https://raw.githubusercontent.com/jgehrcke/covid-19-germany-gae/master/deaths-rki-by-ags.csv",
  ...
  cov_nuts3 <- read.delim(file.path(envrmt$`path_covid-19-germany-gae`, "cases-rki-by-ags.csv"),
    header = TRUE, sep = ",", dec = "."
  )
  ...
  cov_nuts3_deaths <- read.delim(file.path(envrmt$`path_covid-19-germany-gae`, "deaths-rki-by-ags.csv"),
    header = TRUE, sep = ",", dec = "."
  )
  cov_nuts3_deaths$date <- as.POSIXct(cov_nuts3_deaths$time_iso8601, origin = "CET")
  cov_nuts3_deaths <- cov_nuts3_deaths[, -c(
    which(colnames(cov_nuts3_deaths) == "time_iso8601"),
    which(colnames(cov_nuts3_deaths) == "sum_deaths")
  )]
