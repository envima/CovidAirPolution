...
figure_cntry_avg <- lapply(pm_vars, function(pm) {
  ...
  cntry_indv <- cmpldata[[pm]]$it_clstr$clstr
...
saveRDS(figure_cntry_avg, file.path(envrmt$path_figures, "italy_figure_cntry_avg.rds"))
...
figure_gam_lag_set <- lapply(pm_vars, function(pm) {
  cntry_indv <- cmpldata[[pm]]$it_clstr$clstr
  gam_lag_set <- readRDS(file.path(envrmt$path_analysis, paste0("italy_", pm, "_gam_lag_vars_set.rds")))
...
saveRDS(figure_gam_lag_set, file.path(envrmt$path_figures, "italy_figure_gam_lag_set.rds"))
...
figure_gam_lag_mixed_set <- lapply(pm_vars, function(pm) {
  cntry_indv <- cmpldata[[pm]]$it_clstr$clstr
  gam_lag_mixed_set <- readRDS(file.path(envrmt$path_analysis, paste0("italy_", pm, "_gamm_lag_mixed_vars_set.rds")))
...
saveRDS(figure_gam_lag_mixed_set, file.path(envrmt$path_figures, "italy_figure_gam_lag_mixed_set.rds"))
...
saveRDS(model_figure_cumulative_effect, file.path(envrmt$path_figures, "italy_model_figure_cumulative_effect.rds"))
...
map_covid_infections <- lapply(pm_vars, function(pm) {
  ...
  layer <- st_transform(cmpldata[[pm]]$it_nuts3_map, crs = "+init=epsg:25832")
...
saveRDS(map_pm_mean, file.path(envrmt$path_figures, "italy_map_pm_mean.rds"))



# Correlation between PM10 and PM2.5 and influence of outlier
cntry_indv_PM10 <- st_drop_geometry(cmpldata$PM10$it_clstr$clstr[, c("date", "nuts3Code", "pm_mean", "pm_mean_estm", "pm_mean_estm_best", "cases")])
colnames(cntry_indv_PM10) <- c("date", "nuts3Code", paste0(c("pm_mean", "pm_mean_estm", "pm_mean_estm_best", "cases"), "_PM10"))
cntry_indv_PM2.5 <- st_drop_geometry(cmpldata$PM2.5$it_clstr$clstr[, c("date", "nuts3Code", "pm_mean", "pm_mean_estm", "pm_mean_estm_best", "cases")])
...
saveRDS(cor_figure_corr_PM10_PM2.5, file.path(envrmt$path_figures, "italy_cor_figure_corr_PM10_PM2.5.rds"))
